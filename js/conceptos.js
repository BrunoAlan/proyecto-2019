...
function generarImagenes(concepto, cantidadDistractores) {
	...
	imagenCorrecta.src = (link_imagen + conceptos[0] + '.jpg').toLowerCase();
	...
	for (let index = 1; index <= cantidadDistractores; index++) {
		...
		imagen.src = (link_imagen + conceptos[index] + '.jpg').toLowerCase();
...
function generarOpciones(divFase, palabrasFase, cantidadDistractores) {
	...
	aButtonCorrecto.addEventListener(
		'click',
		function() {
			opcionCorrecta();
		},
...
function finEjercicio() {
	...
	setTimeout(function() {
		location.href = 'configuracionConceptos.html';
	}, 1500);
